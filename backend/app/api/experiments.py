...
async def save_config(experiment_id: int, request: SaveConfigRequest):
    """保存试验配置"""
    async with async_session() as session:
        exp = await session.get(Experiment, experiment_id)
        if not exp:
        ...
        # 清除旧配置
        await session.execute(delete(ExperimentModule).where(ExperimentModule.experiment_id == experiment_id))
        await session.execute(delete(ExperimentParam).where(ExperimentParam.experiment_id == experiment_id))
        await session.execute(delete(ExperimentConstraint).where(ExperimentConstraint.experiment_id == experiment_id))

        # 保存模块
        for m in request.modules:
            session.add(ExperimentModule(
        ...
        for c in request.constraints:
            session.add(ExperimentConstraint(
                experiment_id=experiment_id,
